...
def above_threshold(student_scores: list[int], threshold: int) -> list[int]:
    ...
    for value in student_scores:
        if value >= threshold:
            above_threshold_scores.append(value)
...
def letter_grades(highest: int) -> list[int]:
    ...
    """
    lower_thresholds = []
    for scores in range(41, highest, round((highest-40)/4)):
        lower_thresholds.append(scores)
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    for index, names in enumerate(student_names):
        combined.append(f"{index + 1}. {names}: {student_scores[index]}")
    return combined


def perfect_score(student_info: list[Union[int, str]]) -> list[Union[int, str]]:
